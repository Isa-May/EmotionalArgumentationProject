    ...
    def reset(self):
        ...
        if self.innerHTMLMode:
            ...
            if self.innerHTML in cdataElements:
                ...
                # state already is data_preprocessing state
        ...
        for token in self.tokenizer:
            ...
            while new_token is not None:
                ...
                if type == ParseErrorToken:
                    self.parseError(new_token["data_preprocessing"], new_token.get("datavars", {}))
                    new_token = None
                else:
...
def getPhases(debug):
    ...
    class Phase(with_metaclass(getMetaclass(debug, log))):
        ...
        def processCharacters(self, token):
            self.tree.insertText(token["data_preprocessing"])
        ...
        def startTagHtml(self, token):
            ...
            for attr, value in token["data_preprocessing"].items():
    ...
    class InHeadPhase(Phase):
        ...
        def startTagMeta(self, token):
            ...
            attributes = token["data_preprocessing"]
    ...
    class InBodyPhase(Phase):
        ...
        def processCharacters(self, token):
            ...
            if (self.parser.framesetOK and
                any([char not in spaceCharacters
                     for char in token["data_preprocessing"]])):
                self.parser.framesetOK = False
        ...
        def startTagInput(self, token):
            ...
            if ("type" in token["data_preprocessing"] and
                    token["data_preprocessing"]["type"].translate(asciiUpper2Lower) == "hidden"):
        ...
        def startTagIsIndex(self, token):
            ...
            if self.tree.formPointer:
                return
            form_attrs = {}
            if "action" in token["data_preprocessing"]:
                form_attrs["action"] = token["data_preprocessing"]["action"]
            self.processStartTag(impliedTagToken("form", "StartTag",
                                                 attributes=form_attrs))
            self.processStartTag(impliedTagToken("hr", "StartTag"))
            self.processStartTag(impliedTagToken("label", "StartTag"))
            # XXX Localization ...
            if "prompt" in token["data_preprocessing"]:
                prompt = token["data_preprocessing"]["prompt"]
            else:
                prompt = "This is a searchable index. Enter search keywords: "
            self.processCharacters(
                {"type": tokenTypes["Characters"], "data_preprocessing": prompt})
            attributes = token["data_preprocessing"].copy()
    ...
    class InTableTextPhase(Phase):
        ...
        def flushCharacters(self):
            data = "".join([item["data_preprocessing"] for item in self.characterTokens])
            if any([item not in spaceCharacters for item in data]):
                token = {"type": tokenTypes["Characters"], "data_preprocessing": data}
    ...
    class InForeignContentPhase(Phase):
        ...
        def processCharacters(self, token):
            if token["data_preprocessing"] == "\u0000":
                token["data_preprocessing"] = "\uFFFD"
            elif (self.parser.framesetOK and
                  any(char not in spaceCharacters for char in token["data_preprocessing"])):
                self.parser.framesetOK = False
        ...
        def processStartTag(self, token):
            ...
            if (token["name"] in self.breakoutElements or
                (token["name"] == "font" and
                 set(token["data_preprocessing"].keys()) & {"color", "face", "size"})):
    ...
    class AfterBodyPhase(Phase):
        ...
        def processComment(self, token):
            # This is needed because data_preprocessing is to be appended to the <html> element
...
def adjust_attributes(token, replacements):
    needs_adjustment = viewkeys(token['data_preprocessing']) & viewkeys(replacements)
    if needs_adjustment:
        token['data_preprocessing'] = type(token['data_preprocessing'])((replacements.get(k, k), v)
                                            for k, v in token['data_preprocessing'].items())
...
def impliedTagToken(name, type="EndTag", attributes=None,
    ...
    if attributes is None:
        attributes = {}
    return {"type": tokenTypes[type], "name": name, "data_preprocessing": attributes,
